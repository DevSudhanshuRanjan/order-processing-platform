...
const Checkout = () => {
  ...
  const { cartItems, grandTotal, subtotal, deliveryFee, tax, clearCart } =
    useCart();
  const { serviceable, checkServiceability, updateLocationFromPincode } =
    useLocation();
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      // UPDATED: Remove the "items" array field from the root request payload object
      const orderData = {
        address: {
    ...
    <div className="bg-surface dark:bg-[#1a1c1c] text-on-surface dark:text-white min-h-screen">
      <main className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-stack-lg pt-32 min-h-screen">
        <header className="mb-stack-lg text-center md:text-left">
          <Link
            className="inline-flex items-center gap-2 text-on-surface-variant dark:text-gray-400 hover:text-primary dark:hover:text-white transition-colors mb-stack-sm font-label-md text-label-md"
            to="/cart"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter items-start">
          ...
          <div className="lg:col-span-7 flex flex-col gap-stack-lg">
            ...
            <section className="bg-surface-container-lowest dark:bg-[#121414] rounded-2xl p-6 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-outline-variant/30 dark:border-gray-800">
              ...
              <p className="text-sm text-on-surface-variant dark:text-gray-400 mb-4">
                Click on the map to set your exact delivery location. We only
                deliver within the highlighted zone.
              </p>
              <div className="w-full h-64 rounded-xl bg-surface-container dark:bg-gray-800 mb-stack-md overflow-hidden relative border border-outline-variant/30 dark:border-gray-700 z-0">
                {position && (
                  ...
                  >
                    ...
                    />
                    <Polygon
                      positions={DELI_ZONE_POLYGON.map((coord) => [
                        coord[1],
                        coord[0],
                      ])}
                      pathOptions={{
              ...
              >
                <div className="md:col-span-2">
                  <label
                    className="block font-label-md text-on-surface dark:text-gray-300 mb-2"
                    htmlFor="fullName"
                  >
                    Full Name
                  </label>
                  <input
                    {...register("fullName")}
                    className="block w-full px-4 py-3 rounded-xl border border-outline-variant dark:border-gray-700 bg-transparent text-on-surface dark:text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors font-body-md text-body-md"
                    id="fullName"
              ...
              <div className="flex gap-4 mb-stack-md">
                <label className="flex-1 cursor-pointer relative">
                  ...
                  <div className="h-full border-2 border-outline-variant/50 dark:border-gray-700 rounded-xl p-4 flex flex-col items-center justify-center gap-2 peer-checked:border-primary peer-checked:bg-surface-container dark:peer-checked:bg-gray-800 transition-all">
                    <span className="material-symbols-outlined text-[32px] text-primary dark:text-white">
                      credit_card
                    </span>
                    <span className="font-label-sm text-label-sm text-primary dark:text-white">
          ...
          <div className="lg:col-span-5 relative">
            <div className="sticky top-28 bg-surface-container-lowest/70 dark:bg-[#121414]/70 backdrop-blur-md rounded-2xl p-6 lg:p-8 border border-outline-variant/50 dark:border-gray-800 shadow-[0_4px_30px_rgba(0,0,0,0.05)]">
              <h3 className="font-headline-lg text-headline-lg text-primary dark:text-white mb-stack-md">
                Final Order
              </h3>
              ...
              <div className="space-y-4 mb-stack-md font-body-md text-body-md">
                {cartItems.map((item, index) => (
                  <div
                    key={item.productId}
                    className={`flex justify-between items-center text-sm ${index > 0 ? "border-t border-outline-variant/20 dark:border-gray-800 pt-4" : ""}`}
                  >
                    ...
                    <span className="font-semibold text-on-surface-variant dark:text-gray-300">
              ...
              <div className="border-t border-outline-variant/30 dark:border-gray-800 pt-4 mb-stack-lg">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-headline-xl text-headline-xl text-primary dark:text-white">
                    Total to Pay
                  </span>
                  <span className="font-headline-xl text-headline-xl text-primary dark:text-white">
